# Pake Windows & Mac 打包（2 平台 × 国内/国外 × 2 系统）

这个仓库用 **GitHub Actions** 调用 **Pake CLI**，把 2 个平台分别按「国内 / 国外」两个 URL 打包成 Windows 和 Mac 桌面套壳应用（共 8 个产物）。
...
如果你用 Secrets，按下面名称创建 4 个 secret（值就是对应的 URL）：
...
- 推送代码：会自动在 Actions 里构建并上传 8 个 artifacts（4 Windows + 4 Mac）
- 打 tag 发版：创建 `v1.0.0` 这类 tag 并 push，会自动生成 GitHub Release 并把 8 个产物作为附件上传
